...
namespace Pathfinding.Util {
	public static class MovementUtilities {
		 ...
		 * \param cappedSpeed Max speed of the character. In world units per second.
		 ...
		 */
		public static Vector2 ClampVelocity (Vector2 velocity, float cappedSpeed, float slowdownFactor, bool slowWhenNotFacingTarget, Vector2 forward) {
			// Max speed to use for this frame
			var currentcappedSpeed = cappedSpeed * slowdownFactor;
			...
			if (slowWhenNotFacingTarget && (forward.x != 0 || forward.y != 0)) {
				...
				currentcappedSpeed *= directionSpeedFactor;
				currentSpeed = Mathf.Min(currentSpeed, currentcappedSpeed);
				...
				return Vector2.ClampMagnitude(velocity, currentcappedSpeed);
			}
		}

		/** Calculate an acceleration to move deltaPosition units and get there with approximately a velocity of targetVelocity */
		public static Vector2 CalculateAccelerationToReachPoint (Vector2 deltaPosition, Vector2 targetVelocity, Vector2 currentVelocity, float forwardsAcceleration, float rotationSpeed, float cappedSpeed, Vector2 forwardsVector) {
			...
			if (targetVelocity == Vector2.zero) {
				...
				var targetPoint = deltaPosition - normalizedTargetVelocity * System.Math.Min(TargetVelocityWeight * distance * targetSpeed / (currentSpeed + targetSpeed), cappedSpeed*TargetVelocityWeightLimit);
				...
				var finalAcceleration = (targetPoint.normalized*cappedSpeed - currentVelocity) * (1f/TimeToReachDesiredVelocity);
