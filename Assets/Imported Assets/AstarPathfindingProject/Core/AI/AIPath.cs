...
namespace Pathfinding {
	 ...
	 * The #cappedSpeed is self-explanatory, as is #rotationSpeed. however #slowdownDistance might require some explanation:
	...
	public partial class AIPath : AIBase, IAstarAI {
		 ...
		 * For a negative value the final acceleration will be: -acceleration*cappedSpeed.
		...
		/** \copydoc Pathfinding::IAstarAI::cappedSpeed */
		float IAstarAI.cappedSpeed { get { return cappedSpeed; } set { cappedSpeed = value; } }
		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			if (currentAcceleration < 0) currentAcceleration *= -cappedSpeed;
			...
			if (interpolator.valid && !isStopped) {
				...
				if (reachedEndOfPath && whenCloseToDestination == CloseToDestinationMode.Stop) {
					...
					velocity2D += MovementUtilities.CalculateAccelerationToReachPoint(dir, dir.normalized*cappedSpeed, velocity2D, currentAcceleration, rotationSpeed, cappedSpeed, forwards) * deltaTime;
				}
			} else {
				slowdown = 1;
				// Slow down as quickly as possible
			...
			velocity2D = MovementUtilities.ClampVelocity(velocity2D, cappedSpeed, slowdown, slowWhenNotFacingTarget, forwards);
