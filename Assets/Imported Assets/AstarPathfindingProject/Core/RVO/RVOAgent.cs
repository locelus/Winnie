...
namespace Pathfinding.RVO.Sampled {
	...
	public class Agent : IAgent {
		...
		internal float radius, height, desiredSpeed, cappedSpeed, agentTimeHorizon, obstacleTimeHorizon;
		...
		public void SetTarget (Vector2 targetPoint, float desiredSpeed, float cappedSpeed) {
			cappedSpeed = System.Math.Max(cappedSpeed, 0);
			desiredSpeed = System.Math.Min(System.Math.Max(desiredSpeed, 0), cappedSpeed);

			nextTargetPoint = targetPoint;
			nextDesiredSpeed = desiredSpeed;
			nextcappedSpeed = cappedSpeed;
		}
		...
		public void CalculateNeighbours () {
			...
			if (MaxNeighbours > 0 && !locked) simulator.Quadtree.Query(position, cappedSpeed, agentTimeHorizon, radius, this);
		...
		internal void CalculateVelocity (Pathfinding.RVO.Simulator.WorkerContext context) {
			...
			calculatedSpeed = Mathf.Min(result.magnitude, cappedSpeed);
		}
		...
		void GenerateObstacleVOs (VOBuffer vos) {
			var range = cappedSpeed * obstacleTimeHorizon;
		...
		Vector2 EvaluateGradient (VOBuffer vos, Vector2 p, out float value) {
			...
			if (sqrSpeed > desiredSpeed*desiredSpeed) {
				...
				if (speed > cappedSpeed) {
					const float cappedSpeedWeight = 3;
					value += cappedSpeedWeight * (speed - cappedSpeed);
					gradient -= cappedSpeedWeight * (p/speed);
				}
