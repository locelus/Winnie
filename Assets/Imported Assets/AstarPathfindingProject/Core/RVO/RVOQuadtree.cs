...
namespace Pathfinding.RVO {
	...
	public class RVOQuadtree {
		...
		struct Node {
			...
			public float CalculatecappedSpeed (Node[] nodes, int index) {
				if (child00 == index) {
					// Leaf node
					for (var agent = linkedList; agent != null; agent = agent.next) {
						cappedSpeed = System.Math.Max(cappedSpeed, agent.CalculatedSpeed);
					}
				} else {
					cappedSpeed = System.Math.Max(nodes[child00].CalculatecappedSpeed(nodes, child00), nodes[child01].CalculatecappedSpeed(nodes, child01));
					cappedSpeed = System.Math.Max(cappedSpeed, nodes[child10].CalculatecappedSpeed(nodes, child10));
					cappedSpeed = System.Math.Max(cappedSpeed, nodes[child11].CalculatecappedSpeed(nodes, child11));
				}
				return cappedSpeed;
			}
		}
		...
		public void CalculateSpeeds () {
			nodes[0].CalculatecappedSpeed(nodes, 0);
		}
		...
		struct QuadtreeQuery {
			...
			public void QueryRec (int i, Rect r) {
				...
				var radius = System.Math.Min(System.Math.Max((nodes[i].cappedSpeed + speed)*timeHorizon, agentRadius) + agentRadius, maxRadius);
